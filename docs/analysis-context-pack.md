...
**Background（PR 描述对齐）：**`#1381` 聚焦单股分析的当日大盘上下文复用与回退控制，不改变现有日内阶段、日报或状态建模架构。该段为本轮 PR Body/Description 的正式范围声明，须与当前 diff 与 `docs/CHANGELOG.md` [Unreleased] 中的 #1381 条目保持一致，若有偏差请以此为准修订 PR 描述。
**Scope（本轮实现范围）：**`#1381` 仅覆盖后端大盘上下文注入、当日/目标交易日复用控制与保守护栏；本轮不包含独立 API、Web 阶段结果独立展示、四阶段日报结构化持久化或新增日报状态表；未落地项已移至后续计划/Issue。范围对应目录与入口为 `main.py`（调度与 `--no-market-review`）、`src/core/pipeline.py`、`src/services/daily_market_context.py`、`src/analyzer.py`、`src/agent/executor.py`、`src/agent/orchestrator.py`、`src/agent/agents/base_agent.py`、`src/daily_market_context_guardrail.py`。本轮没有 `api/**`、`apps/dsa-web/**`、`apps/dsa-desktop/**`、`scripts/` 或数据库迁移改动，故不应按 Web/API 组件验收。  
**Acceptance Criteria（验收边界）：**本轮仅按后端 runtime 验收，不纳入 PR 过程中的 API/Web UI 独立验收。当前验收路径限制为 `tests/test_main_schedule_mode.py`、`tests/test_daily_market_context.py`、`tests/test_daily_market_context_guardrail.py`、`tests/test_agent_executor.py`。重点覆盖项为：`--no-market-review` 禁止触发大盘复盘生成、单次 schedule 复用同一 `target_date`、多市场上下文加载（`cn,us`）、`daily_market_context` 只在同一次分析主链路注入一次、普通分析与 Agent 路径应用护栏且不泄漏原始 `market_review_payload`。  
